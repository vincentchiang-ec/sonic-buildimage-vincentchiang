...
struct at24_data *accton_at24[MAX_EEPROM_NUM] = {NULL};
...
static const struct i2c_device_id at24_ids[] = {
    ...
    { "accton_24c256",  AT24_DEVICE_MAGIC(262144 / 8,   AT24_FLAG_ADDR16) },
    { "accton_24c64",  AT24_DEVICE_MAGIC(65536 / 8,    AT24_FLAG_ADDR16) },
...
{
    struct at24_data *at24 = NULL;
    int i;
    struct i2c_client *accton_client;
    int ret;
    void *temp = eeprom;

    for(i=0; i<MAX_EEPROM_NUM; i++)
    {
        if(accton_at24[i] != NULL)
        {
            accton_client = at24_translate_offset(accton_at24[i], &offset);
            if((accton_client->adapter->nr == adapter_nr) && (accton_client->addr == addr))
            {
                at24 = accton_at24[i];
                break;
            }
    ...
    if(at24 != NULL)
        ret = at24_read(at24, offset, temp, (size_t)count);
    else
        return -ENXIO;

    return ret;
}
EXPORT_SYMBOL_GPL(at24_read_eeprom);
    ...
    */
    ...
    for(i=0; i<MAX_EEPROM_NUM; i++)
    {
        if(accton_at24[i] == NULL)
        {
            ...
            if(!accton_at24[i])   
            {
                dev_dbg(dev, "accton_at24 is NULL.\n");
                mutex_destroy(&at24->lock);
                return -1;
            }
...
MODULE_DESCRIPTION("Driver for I2C EEPROMs on accton switch, as7327_56x");
