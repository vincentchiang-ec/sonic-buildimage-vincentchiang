...
int fpga_pcie_write( u32 reg, u32 value)
{
    ...
    if(fpga_initialized)
    {
        ...
        I2C_DEBUG("fpga_write_internal 0x%x reg 0x%x\n",value,reg);
    }
    else
    {
        status=-ENXIO;
        I2C_DEBUG("FPGA PCIE is uninitialize!\n");
    }
    return status;
}
...
int fpga_pcie_read( u32 reg)
{
    ...
    if(fpga_initialized)
    {
        ...
        I2C_DEBUG("fpga_read_internal 0x%x reg 0x%x\n",status,reg);
    }
    else
    {
        status=-ENXIO;
...
device_busy:
    I2C_DEBUG(KERN_NOTICE "device_busy, write failed, channel:%d addr:0x%x, length:0x%x, offset:0x%x, stop_step:%d, i2c_stat:0x%x.\n", channel, addr, length, offset, stop_step, i2c_stat);
    ...
    if(fpga_smbus_check_busy(fpga_pci_dev, i2c_cmd_stat_addr, &i2c_stat))
    {
        I2C_DEBUG(KERN_NOTICE "device_busy, fail to issue STOP, i2c_stat:0x%x.\n", i2c_stat);
    }
...
no_ack_response:
    ...
    I2C_DEBUG(KERN_NOTICE "no_ack_response, read failed, channel:%d addr:0x%x, length:0x%x, offset:0x%x, stop_step:%d, i2c_stat:0x%x.\n", channel, addr, length, offset, stop_step, i2c_stat);
...
device_busy:
    ...
    if(fpga_smbus_check_busy(fpga_pci_dev, i2c_cmd_stat_addr, &i2c_stat))
    {
        I2C_DEBUG( "device_busy, fail to issue STOP, i2c_stat:0x%x.\n", i2c_stat);
    }
...
ssize_t fpga_read(struct file *filp, char __user *user_data, size_t size, loff_t *offset)
{
    ...
    struct fpga_reg reg_date;
    int retval = 0;
    fpga_pci_dev = (struct fpga_pci_device *) filp->private_data;

    I2C_DEBUG( "[%s] %s\n", __func__, fpga_pci_dev->name);

    retval = copy_from_user((void *) &reg_date, (const void *) user_data, sizeof(struct fpga_reg));
    if (retval)
    {
        return(-EFAULT);
    }
    reg_date.data = fpga_pcie_read(reg_date.reg);
    retval = copy_to_user((void *) user_data, (const void *) (&reg_date), sizeof(reg_date));
    if (retval)
    {
        return(-EFAULT);
    }

    return(sizeof(reg_date));
}
...
ssize_t fpga_write(struct file *filp, const char __user *user_data, size_t size, loff_t *offset)
{
    ...
    fpga_pcie_write(reg_date.reg, reg_date.data);
...
static int fpga_pci_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    fpga_pci_dev->fpga_udev = device_create(fpga_pci_dev->fpga_class, NULL, fpga_pci_dev->fpga_dev_number, NULL, FPGA_DEV_NAME);
