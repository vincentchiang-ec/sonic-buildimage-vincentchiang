...
class Sfp(SfpBase):
    ...
    PLATFORM = "x86_64-muxi_dcs6500_48z8c-r0"
    HWSKU = "Muxi-dcs6500-48z8c"
    ...
    def __read_eeprom_specific_bytes_rel(self, offset, num_bytes):
        ...
        return eeprom_raw
    def _get_eeprom_bin(self,offset,width):
        ...
        try:
            sysfsfile_eeprom.seek(offset)
            self.eeprom_raw = sysfsfile_eeprom.read(width)
            self.viable = 1
        except Exception:
            print(sysfs_sfp_i2c_client_eeprom_path+" read failed!" )
            self.viable = 0
            self.eeprom_raw = []
            pass
        finally:
    ...
    def __read_eeprom_specific_bytes(self, offset, num_bytes):
        ...
        try:
            if sys.version_info[0] >= 3:
                for n in range(offset, num_bytes+offset):
                    eeprom_raw[n-offset] = hex(self.eeprom_raw[n])[2:].zfill(2)
            else:
                for n in range(offset, num_bytes+offset):
                    eeprom_raw[n-offset] = hex(ord(self.eeprom_raw[n]))[2:].zfill(2)

        except Exception:
            print("eeprom_raw read failed!" )
            self.viable = 0
            self.eeprom_raw = []
            return None
        return eeprom_raw
    ...
    def sfp_twos_comp(self, num, bits):
            try:
                if ((num & (1 << (bits - 1))) != 0):
                    num = num - (1 << bits)
                return num
    ...
    def sfp_calc_temperature(self, eeprom_data, offset, size):
            try:
                msb = int(eeprom_data[offset], 16)
                lsb = int(eeprom_data[offset + 1], 16)

                result = (msb << 8) | (lsb & 0xff)
                result = self.sfp_twos_comp(result, 16)
    ...
    def get_transceiver_temperature(self):
        if self.port_num in range(SFP_PORT_START, QSFP_DD_PORT_END+1):
            self._detect_sfp_type()
            self._dom_capability_detect()
            if not self.dom_supported:
                return None
            transceiver_dom_info_dict = None
            if self.sfp_type == SFP_TYPE:
                #SFP
                if not self.get_presence() :
                    return None
                ...
                if dom_temperature_raw is not None:
                    transceiver_dom_info_dict = self.sfp_calc_temperature(dom_temperature_raw, 0,2)
            elif self.sfp_type == QSFP_DD_TYPE:
                #QSFP-dd case
                if self.sfp_type == QSFP_DD_TYPE:
                    ...
                    if self.dom_temp_supported:
                        ...
                        transceiver_dom_info_dict = self.sfp_calc_temperature(dom_data_raw[start : end], 0,2)
            else:
                #QSFP case
                ...
                if self.dom_temp_supported:
                    ...
                    if dom_temperature_raw is not None:
                        transceiver_dom_info_dict = self.sfp_calc_temperature(
                            dom_temperature_raw, 0,2)

        return transceiver_dom_info_dict

    def _calc_nominal_br(self, br100, br250):
        br100 = int(br100, 16)
        br250 = int(br250, 16)
        rate = 0
        if br100 != 0 and br100 != 0xff:
            rate = br100 * 100
        elif br100 == 0xff:
            rate = br250 * 250
        else:
    ...
    def get_transceiver_info(self):
        ...
        if self.port_num in range(QSFP_PORT_START, QSFP_PORT_END+1):
            transceiver_info_dict = dict.fromkeys(self.info_dict_keys, 'N/A')

            self._get_eeprom_bin(XCVR_TYPE_OFFSET, XCVR_TYPE_WIDTH)
            ret = self._detect_sfp_type()
            if ret != True:
                return None
            eeprom_raw = []
            eeprom_raw = self.__read_eeprom_specific_bytes(XCVR_TYPE_OFFSET, XCVR_TYPE_WIDTH)
            ...
            if self.sfp_type == QSFP_DD_TYPE:
                ...
                if not sfpi_obj:
                    return None
                offset = 0 #QSFP_INFO_OFFSET
                self._get_eeprom_bin(QSFP_INFO_OFFSET, 128)
                ...
                sfp_dom_capability_raw = self.__read_eeprom_specific_bytes_rel(
                ...
                sfp_nominal_bit_rate_raw_100 = self.__read_eeprom_specific_bytes(offset + 12, 1)[0]
                sfp_nominal_bit_rate_raw_250 = self.__read_eeprom_specific_bytes(offset + 94, 1)[0]
                transceiver_info_dict['nominal_bit_rate'] = str(self._calc_nominal_br(sfp_nominal_bit_rate_raw_100, sfp_nominal_bit_rate_raw_250))
        ...
        elif self.port_num in range(SFP_PORT_START, SFP_PORT_END+1) :
            ...
            self._get_eeprom_bin(SFP_INFO_OFFSET, 128)
            sfp_interface_bulk_raw = self.__read_eeprom_specific_bytes(
            ...
            sfp_nominal_bit_rate_raw_100 = self.__read_eeprom_specific_bytes(offset + 12, 1)[0]
            sfp_nominal_bit_rate_raw_250 = self.__read_eeprom_specific_bytes(offset + 66, 1)[0]
            transceiver_info_dict['nominal_bit_rate'] = str(self._calc_nominal_br(sfp_nominal_bit_rate_raw_100, sfp_nominal_bit_rate_raw_250))
    ...
    def get_transceiver_bulk_status(self):
        ...
        if self.port_num in range(SFP_PORT_START, SFP_PORT_END+1) :
            ...
            if not sfpd_obj:
                return None
            self._get_eeprom_bin(XCVR_TYPE_OFFSET, SFP_DOM_OFFSET)
            ...
            offset = 0 #SFP_DOM_OFFSET
            self._get_eeprom_bin(SFP_DOM_OFFSET, 128)
            dom_temperature_raw = self.__read_eeprom_specific_bytes(
                (offset + SFP_TEMPE_OFFSET), SFP_TEMPE_WIDTH)
            ...
            if dom_channel_monitor_raw is not None:
                ...
                return None
        elif self.port_num in range(QSFP_PORT_START, QSFP_PORT_END+1):
            #QSFP case
            transceiver_dom_info_dict = dict.fromkeys(self.dom_dict_keys, 'N/A')
            ...
            if self.sfp_type == QSFP_DD_TYPE:
                ...
                if not sfpd_obj:
                    return None
                offset_xcvr = 0 #QSFP_INFO_OFFSET
                self._get_eeprom_bin(QSFP_INFO_OFFSET, 128)
                # QSFP capability byte parse, through this byte can know whether it support tx_power or not.
                # TODO: in the future when decided to migrate to support SFF-8636 instead of SFF-8436,
                # need to add more code for determining the capability and version compliance
                ...
                self._get_eeprom_bin(QSFP_DOM_OFFSET, 128)
                if self.dom_temp_supported:
                    dom_temperature_raw = self.__read_eeprom_specific_bytes(
                        (offset + QSFP_TEMPE_OFFSET), QSFP_TEMPE_WIDTH)
                    if dom_temperature_raw is not None:
                ...
                if dom_channel_monitor_raw:
                    ...
                    return None
        if self.port_num in range(self.PORT_START, self.PORT_END+1) :
    ...
    def get_transceiver_threshold_info(self):
        ...
        if self.port_num in range(SFP_PORT_START, SFP_PORT_END+1) :
            ...
            if self.sfp_type == QSFP_DD_TYPE:
                ...
                offset = 0 #QSFP_MODULE_UPPER_PAGE3_START

                self._get_eeprom_bin(QSFP_MODULE_UPPER_PAGE3_START+128, 128)
                dom_thres_raw = self.__read_eeprom_specific_bytes(
                    offset+QSFP_MODULE_THRESHOLD_OFFSET-128, QSFP_MODULE_THRESHOLD_WIDTH)
                ...
                dom_thres_raw = self.__read_eeprom_specific_bytes(
                    offset+QSFP_CHANNEL_THRESHOLD_OFFSET-128, QSFP_CHANNEL_THRESHOLD_WIDTH) if self.get_presence() and sfpd_obj else None
    ...
    def get_presence(self):
        ...
        """
        present_path = "{}{}{}{}".format(TRANSCEIVER_PATH, 'eth', self.port_num, '/present')
        #present_path = "{}{}{}".format(FPGA_I2C_PATH, '/module_present_', self.port_num)
    ...
    def get_presence_all(self):
        ...
        present_path = "{}{}".format(TRANSCEIVER_PATH, '/present')
        val1=self._api_helper.read_txt_file(present_path)

        if(val1 is not None):
            val1_str = val1[::-1]
            present = int(val1_str,2)
